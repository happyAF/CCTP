...
|2. 방 생성    |방장이 [방 만들기]를 클릭해 세션 개설, 방 코드 발급                      |
|3. 참여자 입장  |방 코드로 입장 (※ 확장 시: 로드밸런서가 가용 WebSocket 서버로 분산 라우팅)       |
...
`@socket.io/redis-adapter`로 Socket.io 어댑터를 Redis로 교체하였습니다. **현재는 단일 backend 인스턴스로 운영**되고 있으나, 본 어댑터 구성으로 인해 추가 backend 인스턴스를 띄우는 즉시 Redis Pub/Sub을 통해 **모든 서버에 연결된 전체 클라이언트**에 이벤트가 자동 전파됩니다. 따라서 메시지 전파 측면에서는 인스턴스 추가만으로 자동 확장됩니다. 다만 곡 자동 전환 타이머가 backend 프로세스 메모리에 존재하므로, 다중 인스턴스 환경에서는 이 스케줄러를 한 인스턴스만 담당하도록 분산 락(또는 Redis 기반 지연 작업) 처리를 추가해야 합니다.
...
AWS Cloud
   ┌────────────┐
   │ Browser 1  │              ┌──────────────────────────────────┐
   │ Browser 2  │              │           EC2 (t3.small)          │
   │   ...      │              │                                   │
   └──────┬─────┘              │   ┌───────────────────────────┐   │
          │                    │   │     Docker Compose Stack   │   │
          │ ① HTTP / WS        │   │  ┌──────────┐              │   │
          │  (페이지·소켓 연결, │   │  │ frontend │ :8080        │   │
          ├───────────────────►│   │  │ (nginx)  │              │   │
          │  presigned 발급 요청)│  │  └──────────┘              │   │
          │                    │   │       ↕                    │   │
          │                    │   │  ┌──────────┐              │   │
          ...
          │                    └───────────────────────────────────┘
          │
          │ ② S3 직접 PUT / GET  (Presigned URL · 서버 미경유)
          └──────────────────────────────────────►┌─────────┐
                                                   │   S3    │
                                                   │ (미디어) │
                                                   └─────────┘
...
미디어 파일은 backend에서 발급한 Presigned URL로 브라우저가 S3에 직접 PUT/GET 하며, 바이너리는 EC2를 경유하지 않습니다.
...
**확장 가능성**: 본 시스템은 backend가 stateless(상태는 모두 Redis에 위임)이며 Socket.io의 Redis 어댑터로 다중 인스턴스 간 메시지 전파가 자동화되어 있습니다. 따라서 동일한 docker-compose 구성을 추가 EC2 인스턴스에 배포하고 AWS ELB(Application Load Balancer)로 트래픽을 분산하면 메시지 전파 측면의 수평 확장이 가능합니다. ELB의 sticky session 설정으로 WebSocket의 stateful 특성을 처리할 수 있습니다. 다만 현재 곡 자동 전환 타이머가 backend 프로세스 메모리에 존재하므로, 완전한 다중 인스턴스 운영을 위해서는 이 스케줄러를 한 인스턴스만 실행하도록 분산 락(또는 Redis 기반 지연 작업)으로 분리하는 작업이 추가로 필요합니다.
